...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'tube-dist/flatearth-dating-tube/browser');
  ...
  const template = fs
    .readFileSync(
      path.join(join(process.cwd(), 'tube-dist/flatearth-dating-tube/browser'), 'index.html')
    )
    .toString();
      ...
      async (err, html) => {
        ...
        if (
          ...
          const talent = {
            ...
            image: post?.thumbfilename || post?.metaimage || post?.imageUrl || 'https://tube.flatearth.dating/assets/Healing-Tube-Logo.jpg',
          };
          seo.title = talent.name;
          seo.description = strip_html_tags(talent.description);
          seo.image = talent.image;
